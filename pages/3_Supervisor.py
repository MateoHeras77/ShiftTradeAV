        ...
        for req in pending_requests:
            ...
            with st.expander(expander_title):
                ...
                if approve_submitted:
                    ...
                    elif supervisor_password_input_val != "Avianca2025*":
                        st.error("Contraseña del supervisor incorrecta.")
                    elif not has_cover_accepted: # Should be caught by disabled state, but good to double check
...
ShiftTradeAV"""
                                email1 = utils.send_email_with_calendar(
                                    req.get('requester_email'), requester_subject, requester_body,
                                    req, is_for_requester=True
                                )
                                email2 = utils.send_email_with_calendar(
                                    req.get('cover_email'), cover_subject, cover_body,
                                    req, is_for_requester=False
                                )
